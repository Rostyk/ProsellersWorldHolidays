...
- (IBAction)onYesPlease:(id)sender
{
    ...
    [[InAppPurchaseObserver sharedObserver] requestPurchaseWithProductIndentifier:@"HC_FULLVERSION"];
...
- (void)didFinishRestore:(BOOL)status queue:(SKPaymentQueue*)queue error:(NSError*)error
{
    ...
    if (status == YES)
    {
        ...
        for (SKPaymentTransaction *transaction in transactions)
        {
            ...
            if ([product isEqualToString:@"HC_ALL_IMAGES"])
            {
            ...
            else if ([product isEqualToString:@"HC_FULLVERSION"])
            {
                NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
